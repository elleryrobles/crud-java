...
public class UserController {
    ...
    public ResponseEntity<?> nuevo(@Valid @RequestBody NuevoUsuario nuevoUsuario, BindingResult bindingResult) {
        ...
        if (nuevoUsuario.getRoles().contains("1"))
            roles.add(rolService.getByRolNombre(RolNombre.ROLE_ADMIN).get());

        if (nuevoUsuario.getRoles().contains("2"))
    ...
    public ResponseEntity<?> login(@Valid @RequestBody LoginUsuario loginUsuario, BindingResult bindingResult) {
        try {
            if (bindingResult.hasErrors()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ExcepcionErrorResponse("APP" + HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST.value(), "Error en los campos"));
            }
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ExcepcionErrorResponse("APP" + HttpStatus.UNAUTHORIZED.value(), HttpStatus.UNAUTHORIZED.value(), "Nombre de usuario y/o contraseña incorrectos"));
        }
    }
